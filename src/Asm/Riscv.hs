...
instance AsmPrinter Rv64Inst where
  ...
  emitInstAssembly (RV_Ld rs1 rs2 imm) =
    twoArgsInstHelper
      "ld"
      (getRegName rs1)
      ((getImm imm) <> "(" <> getRegName rs2 <> ")")
  emitInstAssembly (RV_Sd rs1 rs2 imm) =
    twoArgsInstHelper
      "sd"
      (getRegName rs1)
      ((getImm imm) <> "(" <> getRegName rs2 <> ")")
  emitInstAssembly (Rv_Mulw rd rs1 rs2) =
    threeArgsInstHelper "mulw" (getRegName rd) (getRegName rs1) (getRegName rs2)
  emitInstAssembly (Rv_Addw rd rs1 rs2) =
    threeArgsInstHelper "addw" (getRegName rd) (getRegName rs1) (getRegName rs2)
  emitInstAssembly (RV_Addiw rd rs imm) =
    threeArgsInstHelper "addiw" (getRegName rd) (getRegName rs) (getImm imm)
  emitInstAssembly (RV_Subw rd rs1 rs2) =
    threeArgsInstHelper "subw" (getRegName rd) (getRegName rs1) (getRegName rs2)
...
commonAsmEmitter (RV_Jal rd label) =
  twoArgsInstHelper "jal" (getRegName rd) (fromString label)
...
threeArgsInstHelper inst a1 a2 a3 =
  inst <> " " <> a1 <> ", " <> a2 <> ", " <> a3
